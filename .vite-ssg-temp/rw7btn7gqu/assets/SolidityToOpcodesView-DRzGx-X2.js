import { defineComponent, ref, onUnmounted, mergeProps, unref, withCtx, createTextVNode, toDisplayString, useSSRContext } from "vue";
...
function WorkerWrapper(options) {
  return new Worker(
    "/assets/solidity.worker-BtCdNLsE.js",
    {
      name: options?.name
    }
  );
...
`);
    ...
    const parsedOpcodes = ref([]);
    const processOutput = (output) => {
      if (output.errors) {
        const criticalErrors = output.errors.filter((e) => e.severity === "error");
        if (criticalErrors.length > 0) {
          error.value = criticalErrors.map((e) => e.formattedMessage).join("\n");
          opcodes.value = "";
          parsedOpcodes.value = [];
          bytecode.value = "";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "h-full flex flex-col p-6 gap-6 overflow-y-auto bg-muted/30" }, _attrs))}><div class="prose dark:prose-invert max-w-none"><h1>Solidity to Opcodes</h1><p class="text-slate-600 dark:text-slate-400"> Compile Solidity code explicitly to view its EVM Opcodes, Bytecode, and execution trace. <br><span class="text-xs text-muted-foreground">Compiles in a Web Worker. Hover over opcodes to see the Execution Stack &amp; Memory.</span></p></div>`);
      if (opcodes.value) {
        _push(`<div class="grid grid-cols-1 md:grid-cols-2 gap-4"><div class="bg-white dark:bg-slate-800 rounded-lg shadow border border-slate-200 dark:border-slate-700 p-4 min-h-[150px]"><h3 class="font-semibold text-xs uppercase text-slate-500 mb-2">Stack</h3>`);
        if (currentState.value) {
          _push(`<div class="font-mono text-xs flex flex-col-reverse gap-1"><!--[-->`);
          ssrRenderList(currentState.value.stack, (item, idx) => {
            _push(`<div class="bg-slate-100 dark:bg-slate-700 p-1 rounded px-2 w-full break-all">${ssrInterpolate(idx)}: 0x${ssrInterpolate(item)}</div>`);
          });
          _push(`<!--]-->`);
          if (currentState.value.stack.length === 0) {
            _push(`<div class="text-slate-400 italic">Empty</div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
        } else {
          _push(`<div class="text-slate-400 text-xs italic">Hover opcode to view state</div>`);
        }
        _push(`</div><div class="bg-white dark:bg-slate-800 rounded-lg shadow border border-slate-200 dark:border-slate-700 p-4 min-h-[150px]"><h3 class="font-semibold text-xs uppercase text-slate-500 mb-2">Memory</h3>`);
        if (currentState.value) {
          _push(`<div class="font-mono text-xs break-all text-slate-600 dark:text-slate-300">${ssrInterpolate(currentState.value.memory && currentState.value.memory !== "" ? currentState.value.memory : "Empty")}</div>`);
        } else {
          _push(`<div class="text-slate-400 text-xs italic">Hover opcode to view state</div>`);
        }
        _push(`</div></div>`);
      ...
      _push(`<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[600px] lg:h-auto flex-1"><div class="flex flex-col gap-4 min-h-0"><div class="bg-white dark:bg-slate-800 rounded-lg shadow border border-slate-200 dark:border-slate-700 flex flex-col h-full overflow-hidden"><div class="p-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between bg-slate-50 dark:bg-slate-900/50"><h2 class="font-semibold text-slate-900 dark:text-white">Solidity Source</h2><div class="flex items-center gap-2"><select class="text-xs p-1 rounded border dark:bg-slate-800 dark:border-slate-700"><!--[-->`);
      ssrRenderList(versions, (v) => {
        _push(`<option${ssrRenderAttr("value", v)}${ssrIncludeBooleanAttr(Array.isArray(compilerVersion.value) ? ssrLooseContain(compilerVersion.value, v) : ssrLooseEqual(compilerVersion.value, v)) ? " selected" : ""}>${ssrInterpolate(v)}</option>`);
      });
      _push(`<!--]--></select></div></div><div class="flex-1 relative">`);
      _push(ssrRenderComponent(CodeEditor, {
        modelValue: sourceCode.value,
        "onUpdate:modelValue": ($event) => sourceCode.value = $event,
        language: "solidity",
        "min-height": "100%",
        class: "absolute inset-0"
      }, null, _parent));
      _push(`</div><div class="p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">`);
      _push(ssrRenderComponent(unref(_sfc_main$1), {
        onClick: compile,
        "aria-label": "Compile Solidity to opcodes",
        loading: isCompiling.value || isCompilerLoading.value,
        class: "w-full"
      }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`${ssrInterpolate(isCompilerLoading.value ? "Loading Compiler..." : "Compile & View Opcodes")}`);
          } else {
            return [
              createTextVNode(toDisplayString(isCompilerLoading.value ? "Loading Compiler..." : "Compile & View Opcodes"), 1)
            ];
          }
        }),
        _: 1
      ...
      if (error.value) {
        _push(`<div class="mt-3 text-red-600 text-xs font-mono bg-red-50 dark:bg-red-900/10 p-2 rounded whitespace-pre-wrap max-h-32 overflow-auto">${ssrInterpolate(error.value)}</div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div></div><div class="flex flex-col gap-4 min-h-0"><div class="bg-white dark:bg-slate-800 rounded-lg shadow border border-slate-200 dark:border-slate-700 flex flex-col h-full overflow-hidden"><div class="p-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50"><h2 class="font-semibold text-slate-900 dark:text-white">EVM Opcodes</h2></div><div class="flex-1 overflow-auto p-4 bg-slate-50 dark:bg-black/20 font-mono text-xs">`);
      if (opcodes.value && parsedOpcodes.value.length > 0) {
        _push(`<div><!--[-->`);
        ssrRenderList(parsedOpcodes.value, (line, idx) => {
          _push(`<div class="group relative flex w-max hover:bg-indigo-50 dark:hover:bg-indigo-900/20 px-1 -mx-1 rounded cursor-help items-baseline"><div class="w-8 text-slate-400 text-[10px] select-none text-right mr-3">${ssrInterpolate(line.pc)}</div><div class="w-24 font-bold text-indigo-600 dark:text-indigo-400">${ssrInterpolate(line.instruction)}</div><div class="text-slate-600 dark:text-slate-400">${ssrInterpolate(line.args)}</div><div class="invisible group-hover:visible absolute left-full top-0 ml-4 z-50 w-64 bg-slate-800 text-white text-[10px] p-2 rounded shadow-lg border border-slate-700 pointer-events-none"><div class="font-bold mb-0.5">${ssrInterpolate(line.instruction)}</div><div>${ssrInterpolate(line.description)}</div><div class="absolute top-2 -left-1 w-2 h-2 bg-slate-800 rotate-45 border-l border-b border-slate-700"></div></div></div>`);
        });
        _push(`<!--]--></div>`);
      } else {
        _push(`<div class="h-full flex flex-col items-center justify-center text-slate-400 text-sm italic">${ssrInterpolate(isCompiling.value ? "Generating Opcodes..." : "Compile source to see opcodes.")}</div>`);
      }
      _push(`</div>`);
      if (bytecode.value) {
        _push(`<div class="h-32 border-t border-slate-200 dark:border-slate-700 p-4 overflow-auto bg-white dark:bg-slate-800"><div class="text-[10px] uppercase font-bold text-slate-500 mb-1">Raw Bytecode</div><div class="font-mono text-[10px] break-all text-slate-600 dark:text-slate-400 select-all">${ssrInterpolate(bytecode.value)}</div></div>`);
      } else {
        _push(`<!---->`);
      }
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("src/views/SolidityToOpcodesView.vue");
