...
class QuantumState {
  ...
  constructor(numQubits: number) {
    ...
    this.amplitudes = Array(size).fill(0).map(() => new Complex(0, 0))
  ...
  H: (target: number, state: QuantumState, numQubits: number): void => {
    ...
    for (let i = 0; i < size; i++) {
      ...
      if (bit === 0) {
        ...
        newState.set(idx0, amp0.multiply(new Complex(1/Math.sqrt(2), 0)).add(amp1.multiply(new Complex(1/Math.sqrt(2), 0))))
      }
    }
...
const addGate = (type: GateType, target: number, control?: number, parameter?: number) => {
  ...
  const hasTargetConflict = lastStep.gates.some(gate => gate.target === target || gate.control === target)
  const hasControlConflict = control && lastStep.gates.some(gate => gate.target === control || gate.control === control)
  
  if (hasTargetConflict || hasControlConflict) {
    circuit.value.push({ gates: [] })
  }
  
  circuit.value[circuit.value[circuit.value.length - 1].gates.push({
    type,
    target,
    control,
    parameter
      ...
      for (const step of circuit.value) {
        for (const gate of step.gates) {
          if (gate.type === 'MEASURE') {
            ...
            // @ts-ignore
            gates[gate.type as keyof typeof gates](gate.target, state, numQubits.value, gate.control, gate.parameter)
          }
        }
      }
            ...
            <CardContent>
              ...
              <div class="mb-4 flex items-center gap-4">
                ...
                <div class="flex items-center gap-2">
                  <Button @click="numQubits = Math.max(1, numQubits - 1)" size="sm" variant="outline">
                    <Minus class="w-4 h-4" />
                  </Button>
                  <span class="font-mono">{{ numQubits }}</span>
                  <Button @click="numQubits = Math.min(8, numQubits + 1)" size="sm" variant="outline">
                    <Plus class="w-4 h-4" />
                  </Button>
                </div>
              </div>
              ...
              <div class="border rounded-lg p-4 bg-surface">
                ...
                <div class="mb-4">
                  ...
                  <div v-for="(step, stepIndex) in circuit" :key="stepIndex" class="mb-2 p-2 border rounded">
                    <div class="flex items-center justify-between mb-2">
                ...
                <div>
                  ...
                  <div class="grid grid-cols-4 gap-2">
                    ...
                    <Button @click="addGate('CNOT', 0, 1)" size="sm" variant="outline" class="text-xs">
                      CNOT
                    </Button>
                    <Button @click="addGate('SWAP', 0, 1)" size="sm" variant="outline" class="text-xs">
                      SWAP
                    </Button>
                    ...
                    <Button @click="addGate('MEASURE', 0)" size="sm" variant="outline" class="text-xs">
                      Measure
                    </Button>
                  </div>
                </div>
        ...
        <Card>
          ...
          <CardContent>
            <div v-if="measurementResults.length > 0" class="space-y-3">
              ...
              <div v-for="result in currentResults" :key="result.qubit" class="p-3 bg-surface-hover rounded-lg border">
